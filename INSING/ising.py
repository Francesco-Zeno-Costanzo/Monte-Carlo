...
for i in range(L):
    for j in range(L):
        if R[i][j]>0:
            plt.errorbar(i, j, fmt='.', color='k')
        else:
            plt.errorbar(i, j, fmt='.', color='w')
plt.title("Simulazione del modello di Ising 2D tramite Metropolis \n Campo magnetico esterno B=%d \n "r"$\beta$=%f" %( H, beta))
...
frames[0].save('animazione1.gif',format='GIF',append_images=frames[:],save_all=True,duration=100,loop=0)
