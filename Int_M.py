...
if np.min(f(x))<0:
    ...
    for i in range(1,N):
        a=rd.uniform(a1,b1)
        b=rd.uniform(np.min(f(x)), 0)
        if b > f(a):
            plt.errorbar(a, b, fmt='.', markersize=1, color='red')
            d+=1
        else:
            plt.errorbar(a, b, fmt='.', markersize=1, color='green')
    I = I + d/N * ((b1-a1)*np.min(f(x)))


dI = ((b1-a1)*(np.max(f(x)) - np.min(f(x))))*np.sqrt((c+d)/N * (1-(c+d)/N))/np.sqrt(N)
print('%f +- %f' %(I, dI))
